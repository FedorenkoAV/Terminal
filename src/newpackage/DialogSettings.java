...
public class DialogSettings extends javax.swing.JPanel {
    ...
    public AddRemoveGlassPane addRemoveGlassPane;
    ...
    public DialogSettings(NewJFrame parent, String portName, int baudRate, int dataBits, int stopBits, int parity) {
        ...
        setSize(480, 129); //задаем размер окна DialogSettings

        parent.setControlsFocusable(false);        
        //glassPane = new GlassPane();//создаем объект с именем glassPane        
        int x = (parent.getWidth()/2) - getWidth()/2;
        int y = (parent.getHeight()/2) - getHeight()/2;
        setBounds(x, y, getWidth(), getHeight());//размещаем DialogSettings по центру родительского окна
        //glassPane.add(this); //добавляем к нему этот компонент (в данном случае DialogSettings)
        //parent.setGlassPane(glassPane); //Устанавливаем свойства glassPane
        //parent.add(glassPane, 0); // дабавляем glassPane к родительскому контейнеру на первое место
        //glassPane.setVisible(true); //делаем glassPane видимой 
        addRemoveGlassPane = new AddRemoveGlassPane();
        addRemoveGlassPane.AddGlassPane(parent, this);
    ...
    private void closeDialog() {
        addRemoveGlassPane.RemoveGlassPane(parent);
